  ...
  return (
    <div className='hidden md:grid md:grid-cols-4 md:h-[15rem] overflow-hidden rounded-2xl md:rounded-3xl md:border text-white md:border-white/10 bg-transparent backdrop-blur-sm shadow-2xl shadow-black/30'>
      {skills.map((item: itemSkill, index) => (
        <div className={`relative text-4xl md:border md:border-white/10 flex items-center justify-center overflow-hidden cursor-pointer transition-all duration-300 ${hoverIcon === index ? 'bg-blue-500/50 shadow-2xl shadow-blue-500/50 ' : 'delay-500'}`}
